...
#include <ctime>
#include "MeshX.h"
#include "LinearSolverX.h"
#include "EigenSolverX.h"
#include "UtilityX.h"
...
void Mesh :: setCurvatureChange0(std::vector<double> rho0)
{
...
// in the range [-scale,scale]
{
cout << "in progress with step:" << scale << "\n";
double mmin = 0.0;
double mmax = 0.0;
double mmean = 0.0;
   for( size_t i = 0; i < faces.size(); i++ )
   {
      // Area of this face is area(i)
      // get handle to current value of rho
      Quaternion rhoQ = 0.;
      ...
      for( int j = 0; j < 3; j++ )
      {
         //vertex of this j of this face is faces[i].vertex[j]
         for(size_t k = 0; k < vertices.size(); k++){
			rhoQ += L(faces[i].vertex[j],k) * newVertices[k];
		}
         
      }
      double ar = area(i);
      rho[i] = scale*(rhoQ.im()).norm()/(4.0*3.0*ar);
      
      Quaternion p1 = newVertices[ faces[i].vertex[0] ];
      Quaternion p2 = newVertices[ faces[i].vertex[1] ];
      Quaternion p3 = newVertices[ faces[i].vertex[2] ];

      if(((( p2-p1 ) * ( p3-p1 )) * rhoQ).re() < 0)
      {
		rho[i] *= (-1.0);
		//cout << "negative" << "\n";
	}else{
...
void Mesh :: setCurvatureChange2( const std::vector<double> v, const double scale )
// sets rho values by interpreting "image" as a square image
// in the range [0,1] x [0,1] and mapping values to the
// surface via vertex texture coordinates -- grayscale
// values in the  range [0,1] get mapped (linearly) to values
// in the range [-scale,scale]
{
   ...
   buildLaplacian();
   vector<Quaternion> rhoQ(vertices.size());
   //rhoQ = L * vertices;
   cout << "rhoQ" << rhoQ[0] << "\n";
   for( size_t i = 0; i < vertices.size(); i++){
	rhoQ[i] = 0;
	for( size_t j =0; j < vertices.size(); j++){
		//cout << "L value" << L(i,j) << "\n";
		//cout << "v value" << vertices[j] << "\n";
		
		rhoQ[i] += L(i,j) * newVertices[j];
		//rhoQ[i] = 0.0;
	}
	//cout << "rhoQ" << rhoQ[i] << "\n";
	}
   ...
   for( size_t i = 0; i < faces.size(); i++ )
   {
      ...
      for( int j = 0; j < 3; j++ )
      {
         //Vector uv = faces[i].uv[j];
         //rho[i] += image.sample( uv.x*w, uv.y*h ) / 3.;
         cout << "rhoQ" << (rhoQ[faces[i].vertex[j]].im()).norm() << "\n";
         rho[i] += (rhoQ[faces[i].vertex[j]].im()).norm();
      }
      m += A[i]*rho[i]/(0.+faces.size());

      // map value to range [-scale,scale]
      //rho[i] = (2.*(rho[i]-.5)) * scale;
   }
   for(size_t i = 0; i < faces.size(); i++){
      //rho[i] = (A[i]*rho[i]-m)/A[i] * scale;
      //rho[i] = scale;	
      rho[i] *= scale;
   }
}
