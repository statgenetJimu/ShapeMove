...
struct SparseMatrix
{
   ...
   void write_matlab(std::ostream &output, const char *variable_name) const
   {
...
struct FixedSparseMatrix
{
   ...
   void construct_from_matrix(const SparseMatrix<T> &matrix)
   {
      ...
      colindex[rowstart[n]] = colindex[rowstart[n]-1]+1;
   }
...
// perform result=matrix*x (optimized)
template<class T>
void multiply(const FixedSparseMatrix<T> &matrix, const std::vector<T> &x, std::vector<T> &result)
{
   assert(matrix.n==x.size());
   ...
   for(unsigned int i=0; i<matrix.n; ++i){
      ...
      unsigned int r0 = matrix.rowstart[i];
      unsigned int r1 = matrix.rowstart[i+1];

      const T* v = &(matrix.value[r0]);

      const unsigned int* k0 = &(matrix.colindex[r0]);
      const unsigned int* k1 = &(matrix.colindex[r1]);

      for(const unsigned int* k = k0; k < k1; k++ )
      {
         R += (*v) * x[*k];
         v++;
      }
